...
Kokoro TTS 输入框还支持直接在单词内部标记效果。标记不会交给模型朗读；程序会先恢复正常拼写，再在生成波形的相应位置寻找低能量切点并插入音素片段。

```text
A containm——ent brea-a-a-a-ch has been detect-t-t-t-t-detected.
```

- `containm——ent`：连续两个或更多长破折号会拖长前一个音素；破折号越多，持续时间越长。
- `brea-a-a-a-ch`：用连字符重复同一个字母或短音节，生成类似 `a-a-a-a` 的局部复读。
- `detect-t-t-t-t-detected`：当前半词后接重复音素，再接完整单词时，会先卡顿，再从该单词开头重新说一遍。
- `Nine-Tailed`、`Fox-3`、`re-containment` 等普通单连字符写法不会触发词内效果。
