...
- **原版语音拼接模式**：保留 SCP:SL / C.A.S.S.I.E. 的片段拼接思路。程序会把输入文本分词，优先匹配词库中的单词和短语，可选叠加按播报时长动态合成的背景提示音，最后导出 WAV。
...
TTS 页面还支持本地模型和本地音色：点击“导入本地模型目录”并选择包含 `config.json`、tokenizer 文件和 `.onnx` 权重的模型目录；点击“导入本地音色（可多选）”可以一次选择多个 Kokoro `.bin` 音色文件，它们会加入“音色”下拉栏并在浏览器 worker 中使用。导入的文件只保留在当前浏览器会话中，不会上传；刷新页面后需要重新选择。不同模型导出的文件格式和音色格式可能不兼容，加载失败时请改用与 `kokoro-js` / Transformers.js 兼容的模型包。

TTS 模式保留原版处理参数：间隔、提前播放、语音延迟、语速、音高、尾音混响，以及可选的精确时长背景提示音。正常模式按句段生成，并可在估算的句内词边界插入间隔；碎片模式会先用 C.A.S.S.I.E. 词库做最长短语匹配，再把剩余普通词组成 2-3 词短碎片生成，以保留更明显的拼接感。生成过程中会显示进度和耗时，也可以中途停止。

TTS 版官方公告模板使用纯文本。可填写字段以普通表单控件展示，预览中会高亮字段，应用后会把完整文本写入 TTS 输入框，方便继续编辑。

固定播报模板中包含“自定义示例：播报指令示意”，会把停顿、故障覆盖、卡顿、拖音、复读和注释语法组合到一段完整公告中，可直接套用后修改。

## C.A.S.S.I.E. 指令效果

原版拼接和 Kokoro TTS 都支持使用显式的 `#{...}` 语法插入控制指令。只有花括号内以 `$` 开头的内容会被视为命令；裸写 `$SLEEP_500`、`.g1` 或普通单词中的连字符不会触发效果。

```text
Attention #{$SLEEP_500} all personnel
SCP-#{$GAP_0}173 has breached
Signal #{$G_1,G_2,G_3,G_4,G_5,G_6} interruption detected
detonation #{#这段内容不会参与解析} sequence cancelled
```

- `#{$SLEEP_毫秒}`：在当前位置插入停顿，范围为 0 到 10000ms。
- `#{$GAP_毫秒}`：精确覆盖当前位置前后的片段间隔，范围为 0 到 10000ms；例如 `SCP-#{$GAP_0}173` 可消除 `SCP` 与编号之间的额外停顿，`SCP-#{$GAP_500}173` 可强制半秒停顿。
- `#{$G_1,G_2,G_3,G_4,G_5,G_6}`：依次播放指定的 G 音频。故障音作为覆盖层叠加到下一段语音的开头，不占用片段间隔，也不推进语音时间线；若命令位于全文末尾，则覆盖上一段语音的末尾。
...
- 背景合成以 `BG_40` 为母带：保留 `0..4s` 的前提示，循环 `4..29s` 的稳定背景噪声，并在目标秒数处接入 `40s` 之后的尾提示和衰减段。
- 应用按完整播报时间线向上取整到秒，动态生成 `BG_AUTO_Ns`；尾提示会精确落在第 `N` 秒，不再受 `BG_4..BG_40` 文件范围限制，因此可以覆盖 40 秒以上的播报。
- 循环边界使用短交叉淡化，背景噪声与尾提示的交界也会淡出 / 淡入，减少直接拼接产生的爆音。
- 原有 `BG_4..BG_40` 和补充的 `BG_14.wav` 仍作为来源资源保留，但网页生成不再逐个选择这些固定长度文件。
