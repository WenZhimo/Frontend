...
Signal #{$G_1,G_2,G_3} interruption detected
detonation #{#这段内容不会参与解析} sequence cancelled
```

- `#{$SLEEP_毫秒}`：在当前位置插入停顿，范围为 0 到 10000ms。
- `#{$G_1,G_2,G_3}`：依次播放指定的 G 音频。故障音作为覆盖层叠加到下一段语音的开头，不占用片段间隔，也不推进语音时间线；若命令位于全文末尾，则覆盖上一段语音的末尾。
- G 音频可使用 `G_1` 到 `G_6`，并可按需要调整顺序或重复，例如 `#{$G_3,G_1,G_3}`。
...
- `#{brea-a-a-a-ch}`：单连字符分隔重复字母或短音节，生成局部卡顿。
