(() => {
  ...
  function finishActiveMove(now) {
    ...
    state.active = null;
    scheduleNextTurn(now, reducedMotion ? 260 : 430);
  }
  ...
  function handleHumanClick(event) {
    ...
    const move = legalMoves().find((candidate) => samePoint(candidate.from, state.selected) && samePoint(candidate.to, cell));
    if (move) {
      state.selected = null;
  ...
  canvas.addEventListener("mousemove", (event) => {
    const cell = cellFromPointer(event);
    const moves = state.selected ? legalMoves() : [];
