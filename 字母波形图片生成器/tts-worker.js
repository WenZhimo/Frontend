...
async function handleGenerate(jobId, message) {
  ...
  if (units.length === 0) {
    post({ type: "error", jobId, message: "没有可生成的 TTS 文本。" });
    return;
  }

  const parts = [];
  const transfers = [];
  post({ type: "progress", jobId, phase: "generate", done: 0, total: units.length });
  ...
  for (let index = 0; index < units.length; index += 1) {
    ...
    try {
      ...
      post({
        ...
        total: units.length,
        currentText: unit.label || unit.text
      });
    ...
    post({
      ...
      total: units.length,
      currentText: units[index].label || units[index].text
    });
  }
