...
const diagnosticToolchainGuardSpecs = [
  ...
  },
  {
    ...
    pattern:
      /const\s+resolutionGateFaceSizes\s*=\s*makeResolutionGateFaceSizes\s*\(\s*smallFaceSize\s*,\s*faceSize\s*\)[\s\S]*?const\s+requestedGroups\s*=\s*parseCsv\s*\(\s*options\.group\s*\?\?\s*options\.groups\s*,\s*\[\s*["']all["']\s*\]\s*\)[\s\S]*?const\s+checkTimeoutMs\s*=\s*parseIntOption\s*\(\s*options\s*,\s*["']timeout-ms["'][\s\S]*?const\s+heavyCheckTimeoutMs\s*=\s*parseIntOption\s*\(\s*options\s*,\s*["']heavy-timeout-ms["'][\s\S]*?resolutionGateFaceSizes\.join\s*\(\s*["'],["']\s*\)[\s\S]*?const\s+selectedChecks\s*=\s*checks[\s\S]*?group:\s*checkGroupForName\s*\(\s*name\s*\)[\s\S]*?timeoutMs:\s*checkTimeoutForName\s*\(\s*name\s*\)[\s\S]*?\.filter\s*\(\s*\(\s*check\s*\)\s*=>\s*groupMatches\s*\(\s*check\.group\s*\)\s*\)[\s\S]*?const\s+timedOutChecks\s*=\s*results\.filter\s*\(\s*\(\s*result\s*\)\s*=>\s*result\.timedOut\s*\)\.map\s*\(\s*\(\s*result\s*\)\s*=>\s*result\.name\s*\)[\s\S]*?passedCheckCount:\s*results\.length\s*-\s*failures\.length[\s\S]*?failedCheckCount:\s*failures\.length[\s\S]*?timedOutCheckCount:\s*timedOutChecks\.length[\s\S]*?timedOutChecks[\s\S]*?timeout:\s*timeoutMs[\s\S]*?function\s+compactMetrics\s*\(\s*name\s*,\s*parsed\s*\)[\s\S]*?["']gpuUsesSphericalCpuReference["'][\s\S]*?["']gpuRectangularPathSkipped["'][\s\S]*?["']regressionCoverageRatio["']/,
  },
  {
